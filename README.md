...
- Device motion / flip triggers: phone placed face down on surface (`DEVICE_FLIPPED_DOWN`) and turned face up back to normal (`DEVICE_FLIPPED_UP`). Evaluated using dual physical verification: Proximity sensor (NEAR) + Gravity/Accelerometer Z-axis ($Z \le -6.5 m/s^2$) + lateral horizontal stability ($\le 6.0 m/s^2$). Uses a 500ms stability debounce window to avoid false triggers during handheld rotations, startup orientation seeding to prevent immediate execution upon engine start, and demand-driven sensor registration (unregisters sensors when no active rules exist, or when screen is off unless the rule explicitly enables screen-off detection with `SENSOR_DELAY_NORMAL` ~5Hz).
- Rule conditions: rules can require multiple conditions to match simultaneously (AND logic) alongside any trigger: Battery level above/below threshold, Charger connected/disconnected, Screen on/off, Connected/disconnected to specific Wi-Fi SSID, Time Window (`TIME_BETWEEN`, e.g. 23:00 - 07:00 with overnight span support), and Days of the Week (`DAYS_OF_WEEK`, Daily, Weekdays, Weekends, or specific day toggles).
